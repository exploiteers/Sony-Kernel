...
*/
static int intel_ce_should_transmit(struct uart_8250_port *up)
{
	unsigned int ier, lsr;
...
#ifdef CONFIG_X86_INTEL_CE
		} else if (intel_ce_should_transmit(up)) {
...
int __init early_serial_setup(struct uart_port *port)
{
	...
	if (p->flags & UPF_FIXED_TYPE)
		serial8250_init_fixed_type_port(&serial8250_ports[port->line],
						p->type);
	set_io_from_upio(p);
	if (port->serial_in)
