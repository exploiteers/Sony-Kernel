...
{
	/* read mac address from module parameter */
	if (mac_addr_hi16 != 0xFFFFFFFF || mac_addr_lo32 != 0xFFFFFFFF) {
		dev->net->dev_addr[0] = (mac_addr_lo32 >> 0) & 0xFF;
		dev->net->dev_addr[1] = (mac_addr_lo32 >> 8) & 0xFF;
		dev->net->dev_addr[2] = (mac_addr_lo32 >> 16) & 0xFF;
		dev->net->dev_addr[3] = (mac_addr_lo32 >> 24) & 0xFF;
		dev->net->dev_addr[4] = (mac_addr_hi16 >> 0) & 0xFF;
		dev->net->dev_addr[5] = (mac_addr_hi16 >> 8) & 0xFF;

		netif_dbg(dev, ifup, dev->net, "MAC address set to fixed\n");
		return;
	}
