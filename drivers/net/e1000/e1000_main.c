...
static void e1000_watchdog(unsigned long data)
{
	...
	/* Intel CE implementation doesn't seem to generate LSC IRQs
	 * (probably because of missing LINK pin). So we just poll status and
	 * force full check if it doesn't match the current state.
	 */
	if (adapter->hw.mac_type == e1000_ce4100 && !adapter->hw.get_link_status) {
		uint16_t phy_data;
		if (e1000_read_phy_reg(hw, PHY_STATUS, &phy_data) ||
		    e1000_read_phy_reg(hw, PHY_STATUS, &phy_data))
			return;

		if (!!(phy_data & MII_SR_LINK_STATUS) != netif_carrier_ok(netdev))
