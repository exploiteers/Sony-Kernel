...
INT RtmpOSNetDevOpsAlloc(
	IN struct net_device_ops **pNetDevOps)
{
        *pNetDevOps = vmalloc(sizeof(struct net_device_ops));
	if (*pNetDevOps)
	{
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
	const struct net_device_ops *pNetDevOps = pNetDev->netdev_ops;
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31)
        struct net_device_ops *pNetDevOps = (struct net_device_ops *)
          pNetDev->netdev_ops;
#endif
