                   ...
                   IN struct iwreq * wrq)
{
  ...
  DBGPRINT(RT_DEBUG_TRACE, ("%s: trace: cmd is %s, len %d\n", __FUNCTION__,
                            cmd, (int) len));
  ...
  } else if ((strncmp("DEBUG", cmd, 5) == 0)) {
    // Set the debug level "DEBUG <int>", see rtmp_def.h for various values
    if (len > 6) {
      sscanf(cmd + 6, "%d", (int *) &RTDebugLevel);
      DBGPRINT(RT_DEBUG_ERROR, ("%s: debug level is now %d\n",
                                __FUNCTION__, (int) RTDebugLevel));
    }
  } else if ((strcmp("STOP", cmd) == 0)) {
    // implement command string "STOP"
    // stop wifi hardware
    wrq->u.data.length = snprintf(cmd, len, "OK");
    result = 0;
