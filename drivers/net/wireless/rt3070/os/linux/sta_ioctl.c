			...
			struct iw_point *data, char *extra)
{
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))
    {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("rt_ioctl_giwscan:: Still scanning\n"));
                // return zero-length list, client will ask again
                data->length = 0;
                return 0;
	}
	 ...
	 case OID_802_11_REMOVE_WEP:
        ...
        case OID_802_11_POWER_MODE:
         ...
         case RT_OID_802_11_TX_POWER_LEVEL_1:
			...
			{
				Status = copy_from_user(&PowerTemp,
                                                        wrq->u.data.pointer,
                                                        sizeof(PowerTemp));
				if (PowerTemp > 100)
	...
	IN	INT					cmd)
{
	...
	pObj = (POS_COOKIE) pAd->OS_Cookie;
        ...
        // signal ENETDOWN if the interface is down.  If a wireless-extension
        // command is specified, ignore interface status
        if((cmd != SIOCSIWPRIV) &&
...
Arguments:
    ...
    (This function is not invoked in GoogleTV, and can be removed)
    ==========================================================================
*/
VOID RTMPIoctlRF(
	IN	PRTMP_ADAPTER	pAd,
...
next:
	...
	DBGPRINT(RT_DEBUG_ERROR, ("<==RTMPIoctlRF, length %d\n\n",
                                  wrq->u.data.length));
}
#endif // RT30xx //
#endif // DBG //
