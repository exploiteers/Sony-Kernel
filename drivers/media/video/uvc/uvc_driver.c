	...
	case UVC_VS_FORMAT_FRAME_BASED:
	{
		...
		if (fmtdesc != NULL) {
			...
			uvc_printk(KERN_INFO,
				"Skipping unknown video format: %pUl\n", guid);
			return -EINVAL;
		}
...
#ifdef CONFIG_QUANTA_PATCH
		if (n == 2) // There are two VS interfaces (EX, QIC1802)
		{
		     ...
		     const struct usb_device_id *id)
{
...
#ifdef CONFIG_QUANTA_PATCH
	dev->inf2_flag = 0; //init flag [Quanta]
#endif
	...
	{	
		...
		atomic_set(&dev2->nstreams, 0);
		atomic_set(&dev2->users, 0);

		dev2->udev = usb_get_dev(udev);
		dev2->intf = usb_get_intf(intf);
		dev2->intfnum = intf->cur_altsetting->desc.bInterfaceNumber;
		dev2->quirks = (uvc_quirks_param == -1)
		    ? id->driver_info : uvc_quirks_param;

		if (udev->product != NULL)
			strlcpy(dev2->name, udev->product, sizeof dev2->name);
		else
...
#ifdef CONFIG_QUANTA_PATCH
	if (dev->inf2_flag  == 1) //[Quanta]
	{
		dev2 = (struct uvc_device *)dev->dev2;	
		uvc_unregister_video(dev2);
	}
#endif
