...
static void read_id(struct nfc_device *dev)
{
	...
	DBG("%d: read_id -> %02x %02x %02x %02x %02x %02x %02x %02x\n", dev->cs,
	    dev->buf[0], dev->buf[1], dev->buf[2], dev->buf[3], dev->buf[4],
	    dev->buf[5], dev->buf[6], dev->buf[7]);
}
...
static u32 do_dma_read(struct nfc_device *dev, u32 irq_mask)
{
	...
	/* HACK!
	 * This is ugly workaround for the following problem:
	 * sometimes DMA_DONE IRQ is triggered but data hasn't arrived
	 * to the memory yet (I saw upto 9us delay).
	 * To catch this situation, we write a magic value at the end of
	 * the buffer and check that it's been over-written by DMA before
	 * continuing. This isn't 100% fool-proof and we also violate all
	 * DMA buffer access rules - thankfully this chip/peripheral is
	 * cache-coherent.
	 */
#define DMA_DONE_MARKER		0xdeadbabe
	volatile u32 *p = (volatile u32*)(dev->buf + (size - 4));
	*p = DMA_DONE_MARKER;
	...
	if (unlikely(*p == DMA_DONE_MARKER)) {
		int i;
		for (i = 0; i < 100; ++i) {
			if (*p != DMA_DONE_MARKER) {
				printk(KERN_DEBUG DRV_NAME "%d: late DMA, %d\n",
				       dev->cs, i);
				goto out;
			}
			ndelay(250);
		...
		printk(KERN_DEBUG DRV_NAME
		       "%d: late DMA hasn't been fixed (maybe normal)\n",
		       dev->cs);
	}
			 ...
			 int page)
{
	...
	if (unlikely(sts & IRQ_STS_ECC_FAILED)) {
		...
		/* Wait for ECC engine to finish */
		wait_irq(dev, IRQ_STS_ECC_DONE);
			       ...
			       const struct pci_device_id *id)
{
	...
	if (unlikely(pci_dma_mapping_error(pdev, dev->buf_dma))) {
	...
	if (nand_scan_ident(&dev->mtd, MAX_BANKS, NULL)) {
		res = -ENXIO;
		goto out_fini_hw;
	}
