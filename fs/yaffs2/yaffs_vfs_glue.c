...
static void yaffs_GrossLock(yaffs_Device *dev)
{
	...
	gross_prio_mutex_lock(&(yaffs_DeviceToLC(dev)->grossLock));
...
static void yaffs_GrossUnlock(yaffs_Device *dev)
{
	...
	gross_prio_mutex_release(&(yaffs_DeviceToLC(dev)->grossLock));
}
...
static int yaffs_reboot_hook(struct notifier_block *nb, unsigned long cmd, void *data)
{
	struct yaffs_LinuxContext *context =
		container_of(nb, struct yaffs_LinuxContext, reboot_notifier);
	struct mtd_info *mtd = yaffs_DeviceToMtd(context->dev);

	printk(KERN_INFO "yaffs: rebooting...\n");
	yaffs_GrossLock(context->dev);
	yaffs_FlushSuperBlock(context->superBlock, 1);
	if (mtd->sync)
		mtd->sync(mtd);
	printk(KERN_INFO "yaffs: superblock has been flushed\n");
...
static int yaffs_BackgroundThread(void *data)
{
	...
	context->reboot_notifier.notifier_call = yaffs_reboot_hook;
	register_reboot_notifier(&context->reboot_notifier);
	...
	unregister_reboot_notifier(&context->reboot_notifier);
	return 0;
}
						...
						void *data, int silent)
{
	...
	gross_prio_mutex_init(&(yaffs_DeviceToLC(dev)->grossLock));
